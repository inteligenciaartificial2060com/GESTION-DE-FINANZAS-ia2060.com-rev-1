document.addEventListener('DOMContentLoaded', function() {
    ...
    const renderizarListado = (movimientos) => {
        ...
        // Ordenar movimientos por fecha para calcular el saldo proyectado
    ...
    const renderizarCalendario = (movimientos) => {
        ...
        // Calcular el índice del día de inicio (Lunes = 0, Domingo = 6)
    ...
    const showDayDetails = (e, dateStr) => {
        ...
        alert(details + "\n\nPara edición detallada, use la Vista Listado."); 
    };
    ...
    // --- Manejo de Eventos CRUD (Expuestos globalmente para el HTML) ---
    ...
    window.editarMovimiento = (id) => {
        ...
        if (movToEdit && !movToEdit.realizado) { // Solo si no está realizado
            // Precargar el formulario
            elements.tipoSelect.value = movToEdit.tipo;
            populateCategories(movToEdit.tipo); 
        ...
        } else if (movToEdit && movToEdit.realizado) {
            alert('Este movimiento está marcado como "Realizado" y no puede ser editado.');
        }
    };
    ...
    elements.clearAllBtn.addEventListener('click', () => {
        if (confirm('ADVERTENCIA: ¿Estás seguro de que quieres borrar TODOS los movimientos y el saldo inicial? Esta acción no se puede deshacer.')) {
            ...
            localStorage.removeItem('currency'); // Borrar la moneda guardada también
    ...
    elements.calendarViewBtn.addEventListener('click', () => {
        ...
        renderizarCalendario(cargarDatos().movimientos); 
    });
    ...
    // --- Manejo del Mes de Ingreso y Calendario ---
    
    const updateMonthDisplay = (date) => {
        // Establece el mes actual para el formulario de ingreso
        currentMonth = new Date(date.getFullYear(), date.getMonth(), 1);
        elements.displayMonth.textContent = currentMonth.toLocaleString('es-ES', { month: 'long', year: 'numeric' });
        
        // Mantener la fecha de hoy, pero si el mes es diferente, poner el primer día.
        const today = new Date();
        const dayToSet = today.getMonth() === date.getMonth() && today.getFullYear() === date.getFullYear() 
                         ? today.getDate() : 1;
        
        // Configurar la fecha de ingreso
        elements.fechaInput.value = new Date(date.getFullYear(), date.getMonth(), dayToSet).toISOString().split('T')[0];
    ...
    const mostrarAlertaSaldoRojo = (fecha, monto) => {
        // Solo avisar una vez al día para evitar spam de voz.
        const lastAlert = localStorage.getItem('last_red_alert');
        ...
        const mensaje = `El saldo proyectado se vuelve negativo (${formatCurrency(monto)}) el día **${fechaFormatted}**. Necesitas un ingreso de al menos **${montoCorregir}** antes de esa fecha.`;

        elements.modalMessage.innerHTML = mensaje;
        elements.modal.classList.add('visible');
        localStorage.setItem('last_red_alert', today); // Guardar la fecha del aviso
    ...
    // Lógica de Reconocimiento de Voz (Mejorada para español)
    ...
    function procesarComandoVoz(comando) {
        ...
        if (tipo) {
            ...
            for (const cat of currentCategories) {
                if (comandoLower.includes(cat.toLowerCase())) {
                    categoria = categories[tipo].find(c => c.toLowerCase() === cat.toLowerCase());
                    break;
                }
            }
        ...
        // Patrones de fecha
        const dateRegex = /(\d{1,2})\s*de\s*([a-záéíóúüñ]+)\s*(?:de\s*(\d{4}))?|el\s*(\d{1,2})/;
        const fechaMatch = comandoLower.match(dateRegex);
        ...
        if (fechaMatch) {
            ...
            if (fechaMatch[1]) { 
                ...
                year = fechaMatch[3] || today.getFullYear(); 
            ...
            } else if (fechaMatch[4]) { // Formato 'el 15'
                day = fechaMatch[4];
        ...
        // Limpiar el comando de palabras clave
        descripcion = descripcion.replace(/ingreso|gasto|(\d+[\.,]?\d*)/gi, '').trim(); 
        if(tipo && categories[tipo]) {
            descripcion = descripcion.replace(new RegExp(categories[tipo].join('|'), 'gi'), '').trim();
        }
        ...
        if (tipo && !isNaN(monto) && monto > 0) {
            ...
            elements.descripcionInput.value = descripcion || (tipo === 'ingreso' ? 'Ingreso Voz' : 'Gasto Voz');
            elements.montoInput.value = monto;
            elements.fechaInput.value = fechaISO;

            alert(`✅ Datos precargados por voz para el ${formatDate(fechaISO)}. Revisa y haz clic en "Agregar Movimiento".`);
        } else {
